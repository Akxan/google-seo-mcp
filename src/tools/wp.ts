    ...
    "wp_update_seo_settings",
    {
      ...
      inputSchema: {
        ...
        socialProfiles: z.object({
          ...
          pinterest: z.string(),
          wikipedia: z.string(),
          other: z.array(z.string()).max(10).describe("Further sameAs URLs with no named field, e.g. TikTok (replaces those; named platforms are kept)."),
        }).partial().optional().describe("The sameAs profiles Yoast puts in the Organization schema; knowledge_graph_check reports on these. Yoast 20+ publishes only facebook, twitter and one list of other URLs, so instagram, linkedin, youtube, pinterest and wikipedia are stored in that list (one URL per platform; empty string removes it)."),
