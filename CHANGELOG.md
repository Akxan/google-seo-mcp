...
### Fixed
- **Instagram, YouTube, LinkedIn, Pinterest and Wikipedia profiles set through `wp_update_seo_settings` never reached the schema.** Yoast 20+ builds an organization's `sameAs` from `facebook_site`, `twitter_site` and the `other_social_urls` list only; the tool wrote the legacy per-platform options (`instagram_url`, `youtube_url`...), which Yoast stores without complaint and never publishes. A live check after setting five profiles found three in the Organization's `sameAs`. Named platforms now go into `other_social_urls`, one URL per platform, the legacy option is cleared, and `wp_get_seo_settings` reports what Yoast actually publishes plus any legacy values it is ignoring.
...
- **Editing a header or footer template left every cached page showing the old version.** `wp_builder_update` purged the edited post's own URL, but a BeTheme template has no page of its own; it is rendered into every page. When the edited post is a template the tool now purges the whole site's page cache, and WP Rocket's used CSS with it: that trimmed stylesheet is a separate per-URL cache computed from the HTML of the day, and a page purge leaves it alone. On a production site a phone icon added to the footer stayed invisible on every page because each page's trimmed CSS was months old and had dropped the rule for a class that did not exist then. (Its `modified` column had moved to the same morning, which made the CSS look fresh; `submitted_at` is the column that says when it was computed.)
- `wp_delete_cache` gains `usedCss`, the same call as WP Rocket's "Clear Used CSS" button, for theme or template changes made outside the tool.
...
### Added
- **`wp_update_seo_settings` sets the organization's contact email and telephone.** Yoast 21+ publishes both in the Organization entity of its schema graph (`org-email`, `org-phone`), but the tool only reached the name, logo and social profiles, so a business's phone number could not get into the structured data search engines and AI assistants read. `organization.email` (validated) and `organization.phone` now do, and `wp_get_seo_settings` reports them.
- **`wp_builder_update` can add an entry to a toggle, tab set or FAQ list.** Writing to the index right after the last entry (`tabs.2` when there are two) creates it with the previous entry's keys, emptied, so the theme finds every field it reads. Any other missing path is still refused, and one bad edit voids the whole batch, so a typo cannot leave a gap in a live list. Needed to give a Spanish footer the WhatsApp line its English counterpart had.
...
### Added
- **The weekly check nobody remembers to run (`seo_digest`).** Every other tool answers a question someone thought to ask; problems appear in the weeks nobody asks. `seo_digest` compares a property against the period before it - totals, the pages and queries that moved, queries that stopped bringing clicks, new ones that started - and opens with a plain-language line saying what happened, so the report reads cold. Set `SEO_MCP_DIGEST_SITES` and the HTTP server runs it weekly by itself (`SEO_MCP_DIGEST_AT`, default `mon:08` UTC), keeps a Markdown copy in `<SEO_MCP_DATA_DIR>/digests/` and, with `SEO_MCP_DIGEST_REPO`, files it as a GitHub issue. The schedule marker survives restarts, so a redeploy never re-sends a digest and a week the server was down is sent late rather than skipped. Search Console position is reported with its direction spelled out, since a smaller number is better.
- **`oauth_list_grants` / `oauth_revoke_grant`.** The OAuth layer could issue access but nothing could show it: seeing who was connected, or cutting one client off, meant opening the SQLite file over SSH. These report each grant's client, scope, call count and last use, and revoke one by id (with `dryRun`). Both are refused to anyone but the operator's own token - a client that arrived through OAuth cannot list or revoke grants, including its own.
- **OAuth for clients that cannot send a header (`SEO_MCP_OAUTH=1`).** ChatGPT's plugin dialog offers *OAuth*, *No authentication* or *Hybrid* only, so `MCP_AUTH_TOKEN` had nowhere to go and the server was unreachable from ChatGPT (the previous release documented an *Access token / API key* option that its UI does not have). The server can now act as its own authorization server: protected-resource and authorization-server metadata (RFC 9728, RFC 8414), dynamic client registration (RFC 7591), authorization code with mandatory PKCE S256, refresh with rotation and revocation (RFC 7009), advertised from a 401 via `WWW-Authenticate: Bearer resource_metadata=…`. Authorization is proof of possession of `MCP_AUTH_TOKEN`, typed once into a browser approval page that names the client and where it sends you back, so no second secret and no user accounts are involved; a read-only checkbox on that page issues a grant whose server registers no write tools. Access tokens last a day, refresh tokens 90 days, both stored only as SHA-256 hashes in `<SEO_MCP_DATA_DIR>/oauth.db`; replaying a rotated refresh token revokes the whole grant, ten wrong tokens from one address suspend the form for fifteen minutes, and approvals, refusals, issued grants and replays each leave an audit line on stderr. Off unless the flag is set, and a static bearer token keeps working exactly as before. `test/oauth-http.mjs` walks the whole flow against a real server on a loopback port and is part of `npm test`.
- **Prompts.** Every argument is optional: a client that lists a prompt without eliciting its arguments (the Claude Code CLI does this) would otherwise fail the call with a validation error. A missing value becomes an instruction the model can act on, and a unit test forbids `undefined` reaching a prompt body. Six workflows are registered as MCP prompts, so clients show them as slash commands and the model follows a written sequence instead of inferring which tool fits: `/traffic_drop`, `/quick_wins`, `/publish_check`, `/site_health`, `/monthly_report`, `/index_bloat`. Each declares the toolsets it needs, so an instance narrowed with `?toolsets=` or `SEO_MCP_TOOLSETS` only advertises workflows it can run (a `gsc,ga4` instance offers four of the six). A unit test asserts every tool named inside a prompt body actually exists, so the guidance cannot drift from the tool list.
...
### Fixed
- **`github_commit_files` now refuses image bytes that travelled through the model.** More than 4 KB of base64 whose header or file extension says image is rejected, with a pointer to `github_commit_image` / `github_commit_attachment`, which fetch and convert on the server. This was the known issue recorded on 2026-09-14, when a client committed a ~67 KB WebP it had written the base64 for itself: the RIFF header was plausible, the file decoded without error, every pixel was noise. Text files, including SVG, are unaffected, and so are small icons.
